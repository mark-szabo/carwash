...
namespace CarWash.Bot.Proactive
{
    ...
    public class DropoffReminderMessage : ProactiveMessage<ReservationServiceBusMessage>
    {
        ...
        public DropoffReminderMessage(CarWashConfiguration configuration, StateAccessors accessors, IAdapterIntegration adapterIntegration, IHostingEnvironment env, BotServices services, TelemetryClient telemetryClient)
            : base(accessors, adapterIntegration, env, services, configuration.ServiceBusQueues.BotDropoffReminderQueue, new Dialog[] { AuthDialog.LoginPromptDialog(), new FindReservationDialog(telemetryClient) }, telemetryClient)
        {
        }
