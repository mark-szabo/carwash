...
namespace CarWash.Bot.Proactive
{
    ...
    public class WashCompletedMessage : ProactiveMessage<ReservationServiceBusMessage>
    {
        ...
        /// <inheritdoc />
        protected override async Task<IActivity[]> GetActivitiesAsync(DialogContext context, ReservationServiceBusMessage message, UserProfile userProfile, CancellationToken cancellationToken = default(CancellationToken))
        {
            Reservation reservation = null;
            try
            {
                var api = new CarwashService(context, cancellationToken);
                reservation = await api.GetReservationAsync(message.ReservationId, cancellationToken);
            }
            catch (Exception e)
            {
        ...
        /// <inheritdoc />
        protected override Task BeginDialogAfterMessageAsync(DialogContext context, ReservationServiceBusMessage message, CancellationToken cancellationToken = default(CancellationToken))
        {
